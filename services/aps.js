...
function getBucket() {
  return process.env.APS_BUCKET || ('dwgvisor-' + CLIENT_ID.toLowerCase().replace(/[^a-z0-9]/g,'').substring(0,18) + '-bkt');
}
...
async function uploadFile(buffer, filename) {
  ...
  const { data } = await axios.put(
    `${APS_BASE}/oss/v2/buckets/${bucket}/objects/${key}`, buffer,
...
async function getManifest(urnB64) {
  const token = await getToken();
  const { data } = await axios.get(`${APS_BASE}/modelderivative/v2/designdata/${urnB64}/manifest`,
    { headers: { Authorization: 'Bearer ' + token } }
  );
  return data;
}
